...
| 项 | 值 |
|---|---|
| 当前版本 | **v1.1.4（2026-09-21）** ｜ 安装脚本自身版本 **v1.1.2**（v1.1.4 **移除自检工具 `ipcheck.sh`**（单机脚本无法完成 NAT 形态区分，实用性有限）；公网 IP 归属改为纯手工判据，**安装脚本未变更**） |
...
## 公网 IP 归属与 NAT 形态（手工判据）

**为什么需要**：在 **NAT 转发型** VPS 上（**1:1 NAT / 弹性公网 IP（EIP）**、**端口映射型共享 NAT（多人 NAT）**、**CGNAT** 都属这一类），公网 IP 由云平台持有、**不在 guest 网卡上**。此时任何 `bind()` 公网 IP 的配置都会失败 —— 典型现象是**入口一切正常（443 在听、REALITY 能握手、日志有 `vless-in` 记录），但打不开任何网页、客户端延迟 `-1`**（根因通常是出站 `sendThrough` 被写成了公网 IP）。

**三条只读命令（10 秒判完，不需要任何脚本）**：

```bash
ip -o addr show | awk '{print $2,$3,$4}'          # ① 内核是否持有该公网 IP
ip route get 1.1.1.1 from <公网IP> | head -1        # ② 报 unreachable ⇒ 不可 bind
ip route | grep default                            # ③ 私网网关 ⇒ NAT 转发型特征
```

- ① 命中 + ② 可 `bind` ⇒ **公网 IP 直配网卡**
- ① 不命中 + ② 不可 `bind` ⇒ **NAT 转发型**（公网 IP 不在你网卡上）

> ⚠️ **"NAT 转发型" ≠ EIP，也 ≠ 独享**。上面三条命令**无法**区分下列三种形态（它们在本机侧的**可观测量完全相同**）：
> **1:1 NAT / EIP**（独享、**全部端口**转发）｜ **端口映射型共享 NAT**（多人**共用**同一公网 IP、只转发商家指定端口）｜ **CGNAT / 共享出口**（无入向映射）。
> 要区分必须从**外部**看（两台机器协作）：
> **D1 全端口覆盖**：本机监听 3~5 个随机高端口、外部逐一连接 ⇒ 全通 = 1:1/EIP；仅指定端口通 = 端口映射型共享 NAT；
> **D2 IP 上是否有"别人的服务"**：外部探测该 IP 常见端口并比对 banner / TLS 证书 ⇒ 出现你不拥有的服务 = 该 IP 被共用；
> **D3 出口 IP vs 商家宣称的 IP**：不等 ⇒ 共享出口 / CGNAT（**相等也不能排除**端口映射型共享 NAT）。
>
> 📌 说明：仓库 v1.1.3 曾随附单机自检脚本 `ipcheck.sh`，**v1.1.4 起已移除** —— 单机脚本只能回答"公网 IP 是否在网卡上"，**无法**完成上述区分，且容易被误读成"EIP / 独享判定"。（该文件仍可在 tag `v1.1.3` 内查看。）
...
| **不确定本机是「公网 IP 直配网卡」还是「NAT 转发型（EIP / 多人 NAT）」** | 跑「公网 IP 归属与 NAT 形态」一节的三条只读命令：`ip -o addr show` 看内核是否持有该公网 IP；`ip route get 1.1.1.1 from <公网IP>` 是否可 `bind`。判定为 **NAT 转发型** ⇒ `sendThrough` 只能填网卡地址或**留空**、监听 IP 用 `0.0.0.0`。⚠️ 该判定**不代表 EIP、也不代表独享**（区分 1:1 NAT 与共享 NAT/CGNAT 需外部探测 D1/D2/D3，见该节） |
...
**v1.1.4（2026-09-21；安装脚本本身未变更，仍为 v1.1.2）**

- **移除自检工具 `ipcheck.sh`**：实测使用价值有限 —— 单机脚本只能回答"公网 IP 是否在本机网卡上"，**无法**区分 1:1 NAT/EIP 与端口映射型共享 NAT、CGNAT（三者在本机侧的可观测量完全相同）；而真正有区分力的方法（**D1/D2/D3**）必须引入**外部视角**，不是单机脚本能完成的。该判定值还容易被误读成"已确认独享 / 就是 EIP"。
- 相关知识**保留为纯手工判据**（README「公网 IP 归属与 NAT 形态（手工判据）」一节，三条只读命令 + D1/D2/D3 说明），`main` 自本版起不再包含 `ipcheck.sh`。
- 版本 `v1.1.3` 中随附的 `ipcheck.sh` 仍可在该 tag 内查看（历史保留）；`enable-xray-stats.sh` 与安装脚本**均未变更**。

**新增工具 `ipcheck.sh`（2026-09-21；安装脚本本身未变更，仍为 v1.1.2）**

- 新增**只读**自检工具 `ipcheck.sh`（**该文件已在 v1.1.4 移除**，历史版本见 [tag v1.1.3](https://github.com/shirasawatop/xray-vless-reality-install/blob/v1.1.3/ipcheck.sh)）：判定本机是「公网 IP 直配网卡」还是「**公网 IP 不在网卡上（即 NAT 转发型：可能是 1:1 NAT/EIP，也可能是端口映射型共享 NAT/CGNAT）**」，并输出 `sendThrough` / 监听 IP 的硬约束建议。
...
- 实测（两台机型对照）：**NAT/EIP 型 → `eip`**（该判定值自 v1.1.4 起更名为 `nat-forwarded`；实测 `<公网IP> 不在本机网卡上，内核也不接受其作为源地址`）；**公网 IP 直配型 → `direct`**（`<公网IP> 出现在本机网卡地址中`）。
...
- 验证：在 NAT 型实机实测 `addr_on_this_host`：`<私网IP>` → 在网卡上；`<公网IP>`/`127.0.0.1` → 不在；模拟阶段 19 判定结果正确；`bash -n` 通过。

**v1.1.1（2026-09-21）**

- **修复（严重，实机复现）：脚本在「阶段 1 检测 IP」处静默退出** —— 现象为界面打印 `正在检测系统网络配置...` 后**直接回到提示符**，无任何报错。
  - 根因：`set -e` 与「命令替换内以 `grep` 结尾的管道」组合 —— 无匹配时最后一条 `grep` 返回 1 ⇒ 整条赋值失败 ⇒ 脚本退出；且该命令的 `stderr` 已被 `2>/dev/null` 丢弃，故**完全静默**。
  - 触发条件：**主机没有公网 IPv6**（`eth0` 仅有 `fe80::/64` 链路本地）时，IPv6 检测分支必然命中。实测：NAT 型 VPS `sadidc` 必现，而带公网 IPv6 的 `lycheen` 不现。
  - 修复：IPv4/IPv6 检测的命令替换末尾加 `|| true`；`xray mlkem768` 的 `Seed:`/`Client:` 解析同步改为 `sed` 宽松解析（原 `grep` 同样会静默退出）。
- **新增：NAT 型 VPS 提示** —— 检测到的地址为私网（`10./172.16-31./192.168.`）时，主动提示：① 「监听IP」保持 `0.0.0.0`（填公网 IP 会绑定失败）；② 出口可选私网地址（`sendThrough` 用真实网卡地址，出网经 NAT）；③ 订阅链接的公网 IP 由 Cloudflare 探测，不受影响。
- README 排障表新增上述两条（含旧版本**应急热修**命令）；无其它行为变更。
