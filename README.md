...
| 判定 | 含义 | 输出示例 |
|---|---|---|
| `direct` / `direct-like` | 公网 IP **就在网卡上**（可 `bind`） | 实测：`<公网IP> 出现在本机网卡地址中` |
| `eip` / `eip-by-gw` | **1:1 NAT / EIP**：公网 IP 独享但不在网卡上 | 实测：`<公网IP> 不在本机网卡上，内核也不接受其作为源地址` |
...
- 实测（两台机型对照）：**NAT/EIP 型 → `eip`**（`<公网IP> 不在本机网卡上，内核也不接受其作为源地址`）；**公网 IP 直配型 → `direct`**（`<公网IP> 出现在本机网卡地址中`）。
